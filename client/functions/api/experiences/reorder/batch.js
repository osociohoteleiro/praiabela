...
export async function onRequestPut({ request, env }) {
  ...
  try {
    const stmt = env.DB.prepare('UPDATE experiences SET display_order = ? WHERE id = ?')
    await env.DB.batch(items.map((item, i) => stmt.bind(i, item.id)))
    return json({ message: 'Ordem atualizada com sucesso' })
  } catch (err) {
    console.error('Reorder experiences error:', err)
    return serverError('Erro ao reordenar experiências')
  }
}
